...
/*--------------------------------------------------------------------------
 ...
 |-------------------------------------------------------------------------*/
namespace willphp\middleware\build;
use willphp\config\Config;
/**
 * 中间件处理
 * Class Base
 * @package willphp\middleware\build
 */
class Base {
	protected $params; //中间件传递参数
	/**
	 * 执行中间件
	...
	public function set($name, $mode = []) {
		...
		if (!$mode) {
			...
			$get_action = Config::get('middleware.get_action', ''); //获取当前的控制器方法
			$action = is_callable($get_action)? strtolower(call_user_func($get_action)) : 'index'; //当前控制器方法
